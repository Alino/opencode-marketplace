...
```
github.com/owner/repo/ref_v1.0.0/     ← cloned marketplace repo
ssh.dev.azure.com/v3/org/proj/repo/ref_main/
local/path/to/local/repo/ref_default/
```

Each `ref_*` or `sha_*` subdirectory is an independent clone so pinning different consumer repos to different versions never causes conflicts.
